...
def test_horizon_6h_resolution_1h():
    ...
    estimator = Persistence2Estimator(
    ...
    expected = np.percentile([[24 * i + j for i in range(10)] for j in range(6)], quantile_levels, axis=1).T
    train_and_check(estimator, data, issue_times, expected)

    issue_times = pd.date_range("2018-01-03 07", "2018-01-08 07", freq="1d")
    expected = np.percentile([[24 * i + j for i in range(2, 8)] for j in range(7, 13)], quantile_levels, axis=1).T
    train_and_check(estimator, data, issue_times, expected)

    issue_times = pd.date_range("2018-01-01 00", "2018-01-10 18", freq="6h")
    expected = np.percentile([[6 * i + j for i in range(40)] for j in range(6)], quantile_levels, axis=1).T
    train_and_check(estimator, data, issue_times, expected)
